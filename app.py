...
def search_jobs():
    ...
    # 2. APIFY LINKEDIN JOBS FETCH
    APIFY_API_KEY = os.environ.get("APIFY_API_KEY")
    
    if not APIFY_API_KEY:
        print("❌ APIFY_API_KEY not set!")
        return jsonify({"error": "Apify API key not configured"}), 500
    ...
    # Build LinkedIn search URLs from job titles
    search_urls = []
    for title in search_queries:
        encoded_title = requests.utils.quote(title)
        url = f"https://www.linkedin.com/jobs/search/?keywords={encoded_title}&location=Netherlands&f_TPR=r604800"
    ...
    for job in raw_results:
        ...
        # Map Apify fields to existing structure
        final_jobs.append({
            "title": job.get('title'),
            "company": job.get('companyName', 'Unknown'),
            "location": job.get('location', 'Netherlands'),
            "job_url": link,
            "description": job.get('descriptionText', job.get('title', '')),
            "match_score": 0,
